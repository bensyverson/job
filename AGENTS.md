...
- **Open with `job status`.** At session start, `job status` (no arg) is both the identity check and the landscape briefing: preamble with the configured default identity and strict-mode state, then one row per top-level task with its own subtree counts, then `Next:` for the globally-next claimable leaf, `Stale:` for any abandoned claims past their TTL, and `Decision:` for any open tasks labeled `decision` (human decisions pending). Agents reading the docs to find "what's going on here" should run `status` before doing anything else. `job status <id>` scopes the same renderer to a subtree and also appends the actionable task list (same as `list <id>`); `summary [id]` still works but is a deprecated alias.
- **Prefer canonical forms; aliases warn.** `block add` / `block remove` are the canonical block-edge verbs (kept the `by` keyword: `block add X by Y`). The shorter `block X by Y` and `unblock X from Y` still work as aliases but emit a one-line stderr deprecation notice on every call. Same for `ls` (use `list`), `show <id>` (use `info <id>`), and `summary [id]` (use `status [id]`). When you write tooling or examples, prefer the canonical form.
- **`-m` is the free-text body short flag** across `note`, `done`, and `cancel`. All three echo the stored body back in the ack (`  note: N chars · "preview"` on `done`; `  reason: N chars · "preview"` on `cancel`; `Noted: <id> · N chars · "preview"` as the `note` ack itself) so you get immediate confirmation the right text landed. The convention extends to other commands' obvious mappings: `-d` for `--desc`, `-t` for `--title`, `-l` for `--label`, `-p` for `--parent`, `-n` for `--dry-run`, `-s` for `--since`, `-y` for `--yes`. Letters reserved by long-standing convention (`-r` recursive, `-f` force, `-v` verbose, `-h` help) are intentionally not reused for unrelated semantics.
- **You rarely need `heartbeat`.** Any write to a task you currently hold (`note`, `edit`, `label add`, `label remove`) auto-extends the claim TTL to `now + 30m`. So the usual cadence — check in with `note` on progress, keep working — is enough to keep the claim alive. Reach for `heartbeat` only when you are genuinely holding a claim without writing to it (a long "thinking" pause, for example). Extensions never shorten a longer explicit duration.
