...
- **Open with `job status`.** At session start, `job status` (no arg) is both the identity check and the landscape briefing: preamble with the configured default identity and strict-mode state, then one row per top-level task with its own subtree counts, then `Next:` for the globally-next claimable leaf and `Stale:` for any abandoned claims past their TTL. Agents reading the docs to find "what's going on here" should run `status` before doing anything else. `job status <id>` scopes the same renderer to a subtree; `summary [id]` still works but is a deprecated alias.
