...
func TestNextAll_DescendsIntoNestedLeaves(t *testing.T) {
	dbFile := setupCLI(t)
	db := openTestDB(t, dbFile)
	grand := mustAdd(t, db, "", "Grandparent")
	parent := mustAdd(t, db, grand, "Parent")
	leaf := mustAdd(t, db, parent, "Leaf")
	db.Close()
...
func TestNextAll_ParentWithOnlyCanceledChildrenIsLeaf(t *testing.T) {
	...
	if _, _, _, err := runCancel(db, []string{child}, "obsolete", false, false, false, testActor); err != nil {
...
func TestClaim_RefusesParentWithOpenChildren(t *testing.T) {
	...
	parent := mustAdd(t, db, "", "Parent")
	_ = mustAdd(t, db, parent, "Child 1")
	_ = mustAdd(t, db, parent, "Child 2")

	err := runClaim(db, parent, "1h", "alice", false)
	if err == nil {
...
func TestClaim_ParentWithOnlyClosedChildrenWorks(t *testing.T) {
	db := setupTestDB(t)
	parent := mustAdd(t, db, "", "Parent")
	doneChild := mustAdd(t, db, parent, "Done")
	canceledChild := mustAdd(t, db, parent, "Canceled")
	mustDone(t, db, doneChild)
	if _, _, _, err := runCancel(db, []string{canceledChild}, "obsolete", false, false, false, testActor); err != nil {
		t.Fatalf("cancel: %v", err)
	}

	if err := runClaim(db, parent, "1h", "alice", false); err != nil {
...
func TestClaim_RefusesParentEvenWithForce(t *testing.T) {
	...
	err := runClaim(db, parent, "1h", "alice", true)
	if err == nil {
...
func TestDone_LastOpenChildAutoClosesParent(t *testing.T) {
	db := setupTestDB(t)
	parent := mustAdd(t, db, "", "Parent")
	child := mustAdd(t, db, parent, "Only Child")

	if _, _, err := runDone(db, []string{child}, false, "", nil, testActor); err != nil {
		t.Fatalf("done child: %v", err)
	}

	p := mustGet(t, db, parent)
	if p.Status != "done" {
...
func TestDone_AutoCloseCascadesUpTree(t *testing.T) {
	db := setupTestDB(t)
	grand := mustAdd(t, db, "", "Grand")
	parent := mustAdd(t, db, grand, "Parent")
	leaf := mustAdd(t, db, parent, "Leaf")

	if _, _, err := runDone(db, []string{leaf}, false, "", nil, testActor); err != nil {
...
func TestDone_CanceledSiblingsDoNotBlockAutoClose(t *testing.T) {
	db := setupTestDB(t)
	parent := mustAdd(t, db, "", "Parent")
	canceled := mustAdd(t, db, parent, "Canceled")
	open := mustAdd(t, db, parent, "Open")
	if _, _, _, err := runCancel(db, []string{canceled}, "obsolete", false, false, false, testActor); err != nil {
		t.Fatalf("cancel: %v", err)
	}

	if _, _, err := runDone(db, []string{open}, false, "", nil, testActor); err != nil {
...
func TestDone_DoesNotAutoCloseWhenSiblingOpen(t *testing.T) {
	...
	parent := mustAdd(t, db, "", "Parent")
	c1 := mustAdd(t, db, parent, "C1")
	_ = mustAdd(t, db, parent, "C2") // still open

	if _, _, err := runDone(db, []string{c1}, false, "", nil, testActor); err != nil {
...
func TestAdd_ChildToClaimedParent_AutoReleasesClaim(t *testing.T) {
	...
	if _, err := runAdd(db, parent, "Child", "", "", "alice"); err != nil {
...
func TestAdd_SecondChild_IsIdempotent(t *testing.T) {
	...
	if _, err := runAdd(db, parent, "First", "", "", "alice"); err != nil {
		t.Fatalf("add first: %v", err)
	}
	// Parent now auto-released. Adding a second child should not error or
	// try to re-release.
	if _, err := runAdd(db, parent, "Second", "", "", "alice"); err != nil {
...
func TestDone_EventLog_RecordsAutoCloseAttribution(t *testing.T) {
	...
	if _, _, err := runDone(db, []string{child}, false, "", nil, "alice"); err != nil {
		t.Fatalf("done child: %v", err)
	}
